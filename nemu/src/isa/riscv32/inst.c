...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(dest) = imm); 
	...
	INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , I, bool success = false; s->dnpc = ECALL+4);
	INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs  , I, vaddr_t t = CSR; R(dest) = CSR; CSR = (t | src1));
